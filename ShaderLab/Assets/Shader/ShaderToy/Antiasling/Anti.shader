...
Shader "shadertoy/AA Line" { 
	Properties{
		...
		OutlineWidth ("Outline Width", Range(0, 0.1)) = 0.01
	...
	CGINCLUDE    
		...
		const float seg = 3.0;
		const float segwidth = 1.0/(2.0*3.0-1.);
       ...
       //   precision highp float;
        ...
        float mline(vec2 pos, vec2 point1, vec2 point2, float width) {   		
		...
		vec4 main(vec2 fragCoord) {
			...
			float d = mline(pos, point0, point1, _LineWidth);
			d = min(d, mline(pos, point1, point2, _LineWidth));
			d = min(d, mline(pos, point2, point3, _LineWidth));
			d = min(d, mline(pos, point0, point2, _LineWidth));
			d = min(d, mline(pos, point0, point3, _LineWidth));
			d = min(d, mline(pos, point1, point3, _LineWidth));
			...
			if (originalPos.x < split.x) {
				col = mix(_OutlineColor.rgb, col, step(0, d - OutlineWidth));
				col = mix(_LineColor.rgb, col, step(0, d));
			} else if (originalPos.y > split.y) {
				float w = _Antialias;
				col = mix(_OutlineColor.rgb, col, smoothstep(-w, w, d - OutlineWidth));
