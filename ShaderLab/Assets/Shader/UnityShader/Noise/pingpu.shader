    ...
    SubShader
    {
        ...
        pass
        {
            ...
            float SeamlessNoise( float x, float y, float dx, float dy, float xyOffset ) {
                ...
                float t = y;
                float MathfPI=3.14;
                float nx = xyOffset + cos(s * 2.0f * MathfPI) * dx / (2.0f * MathfPI);
                float ny = xyOffset + cos(t * 2.0f * MathfPI) * dy / (2.0f * MathfPI);
                float nz = xyOffset + sin(s * 2.0f * MathfPI) * dx / (2.0f * MathfPI);
                float nw = xyOffset + sin(t * 2.0f * MathfPI) * dy / (2.0f * MathfPI);

                // return simplex_noise(nx, ny, nz, nw);
                return simplex_noise(float2(nx,ny));
            }
